...
class YOLOv5App:
    def __init__(self, root):
        ...
        # HTTP 스트림 URL
        # self.camera_url = "http://<ip>:<port>/video"  # IP에 맞춰서 수정@@@@@@@@@@@@@
        self.camera_url = 'http://118.39.27.74:4747/video' #hntwifi_02에 연결
    ...
    def start_detection(self):
        if self.webcam is None:
            ...
            self.webcam = cv2.VideoCapture(self.camera_url)  # HTTP 스트림 열기

            if not self.webcam.isOpened():
                messagebox.showerror("Error", "HTTP 스트림에 연결할 수 없습니다. URL을 확인하세요.")
                self.webcam = None
                return
